...
(function(){
  ...
  function rel(name){const f=baseFactions.find(x=>x.name===name); const v=(state.factionStatus?.[name] ?? f?.status ?? 0); return v>0?'good':v<0?'bad':'neutral'}
  function statusText(name){const f=baseFactions.find(x=>x.name===name); const v=(state.factionStatus?.[name] ?? f?.status ?? 0); return v>0?`+${v}`:String(v)}
  function homeHTML(){
    const c0=state.clocks?.[0]||{name:'Grinders retaliate',size:6,filled:2};
    const c1=state.clocks?.[1]||{name:'Wraith suspicion',size:4,filled:0};
  ...
  window.render=function(){
    if(state.page!=='home'){return priorRender();}
    renderNav();
