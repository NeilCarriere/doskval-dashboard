// Doskval NPC portraits v9 — exact pixel crop from 100-face sheet; prevents adjacent-cell bleed.
(function(){
  const SPRITE='assets/F7096ECF-A363-4258-98AA-5C2DD3364C38.png?v=curated100d';
  ...
  const CELL={
    'Quellyn':86,'Flint':88,'Stazia':59,'Malista':69,'Telda':91,'Frake':93,'Fitz':11,
  ...
  function apply(){
    ...
    document.querySelectorAll('.npcTile').forEach(tile=>{
      ...
      if(!name||!box)return;
      if(box.dataset.portraitV9===name)return;
      box.innerHTML='';box.appendChild(make(name,false));box.dataset.portraitV9=name;
    });
    ...
    if(name&&hero){hero.style.position='relative';hero.style.overflow='hidden';
      if(hero.dataset.portraitV9!==name){hero.innerHTML='';hero.appendChild(make(name,true));const lab=document.createElement('span');lab.className='vizLabel';lab.textContent='CAMPAIGN VISUALIZATION';hero.appendChild(lab);hero.dataset.portraitV9=name;}
    }
