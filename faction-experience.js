...
(function(){
  ...
  let webMode='campaign', webFocus='The Sneaks', edgePair=['The Sneaks','The Crows'], ledgerSelected='The Crows', ledgerSearch='', ledgerCategory='All';
  ...
  function ledgerCard(f){const d=loreFor(f),v=stand(f);return `<button class="ledgerCard ${f.name===ledgerSelected?'selected':''}" data-ledger="${E(f.name)}"><div class="ledgerCardTop">${emblem(f)}<div><small>${E(sourceFor(f))}</small><h2>${E(f.name)}</h2><p>Tier ${E(f.tier)} · ${E(f.hold)} hold</p></div></div><p>${E(d.summary||f.canon)}</p><div class="ledgerFoot"><span class="${relClass(standWord(v))}">${E(standWord(v))}</span>${relationPips(v)}</div></button>`;}
  function ledgerDossier(f){const d=loreFor(f),v=stand(f);return `<aside class="ledgerDossier">${emblem(f,true)}<small>${E(sourceFor(f))} · ${E(categoryFor(f))}</small><h2>${E(f.name)}</h2><div class="tierStrip"><span>TIER <b>${E(f.tier)}</b></span><span>${E(f.hold)} HOLD</span></div><p>${E(d.summary||f.canon)}</p>${f.campaign?`<div class="campaignBrief"><b>SNEAKS CAMPAIGN</b><p>${E(f.campaign)}</p></div>`:''}<div class="standingEditor"><label>STANDING WITH THE SNEAKS</label><div><button data-fdelta="-1" aria-label="Reduce standing">−</button><b class="${relClass(standWord(v))}">${E(standWord(v))} ${v>0?'+':''}${v}</b><button data-fdelta="1" aria-label="Increase standing">+</button></div></div><h3>IMPORTANT FACES</h3>${d.npcs?.length?d.npcs.map(n=>`<p class="faceLine"><b>${E(n[0])}</b> — ${E(n[1])}</p>`).join(''):'<p class="emptyIntel">No named faces in the current source notes.</p>'}<h3>ALLIES</h3>${chips(d.allies,'friend')}<h3>ENEMIES</h3>${chips(d.enemies,'enemy')}<h3>FACTION CLOCKS</h3>${clockRows(f)}<div class="inspectorActions"><button data-focus="${E(f.name)}">OPEN IN FACTION WEB</button><button class="addFactionClock" data-faction="${E(f.name)}">ADD CAMPAIGN CLOCK</button></div></aside>`;}
  ...
  function bindFactionUI(){
    ...
    document.querySelectorAll('[data-ledger]').forEach(b=>b.onclick=()=>{ledgerSelected=b.dataset.ledger;state.page='factions';save();renderFactionLedger()});
